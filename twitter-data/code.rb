...
puts '1. Each username followed by its description. If there is no description (i.e., `nil` then print out "NA" instead):'
twitter_data.each do |user|
  user.each do |username, data|
    puts "#{username}: #{data['description'] ? data['description'] : 'NA'}"
  end
end
puts
...
twitter_data.each do |user|
  user.each do |username, data|
    puts "#{username}'s latest tweet was #{data['last twenty tweets'][0].length} characters long."
  end
end
puts
# 4. Each username followed by the total character count used in that user's last twenty tweets.
puts "4. Each username followed by the total character count used in that user's last twenty tweets."

twitter_data.each do |user|
  user.each do |username, data|
    total = data['last twenty tweets'].inject(0) { |sum, tweet| sum += tweet.length}
    puts "#{username}'s used #{total} characters in their last twenty tweets."
  end
end
puts
# 5. Write Ruby code to answer each of the following questions about our Twitter data.
puts '5. Write Ruby code to answer each of the following questions about our Twitter data.'
...
twitter_data.each do |user|
  user.each do |username, data|
    if data['number of followers'] > most_followers
      most_popular = username
      most_followers = data['number of followers']
    end
  end
end
puts "#{most_popular} has the most followers at #{most_followers} followers."
puts
...
twitter_data.each do |user|
  user.each do |username, data|
    if data['number of friends'] > most_friends
      most_friendly = username
      most_friends = data['number of friends']
    end
  end
end
puts "#{most_friendly} has the most friends at #{most_friends} friends."
puts
...
twitter_data.each do |user|
  user.each do |username, data|
    if data['number of tweets'] > most_tweets
      most_vocal= username
      most_tweets = data['number of tweets']
    end
  end
end
puts "#{most_vocal} tweets the most at #{most_tweets} tweets"
puts
# * Which users have a `description` listed in `twitter_data`?
puts 'Which users have a `description` listed in `twitter_data`?'
twitter_data.each do |user|
  user.each do |username, data|
    puts "#{username}" if data['description']
  end
end
puts
...
twitter_data.each do |user|
  user.each do |username, data|
    puts "#{username}" if data['location']
  end
end
